...
//
// Boots the app's data layer (db.js, Finance section included) DIRECTLY — no Electron, no
...
try {
  ...
  // Gates 4 and 5 used to assert the opposite of this: that Finance kept its own
  // catalog table and never appeared in LOOKUP_CATEGORIES. Migration 060 folded
  // that catalog into the shared lookup_codes, so what needs guarding now is that
  // the fold is complete — the categories are gated like every other one, and
  // nothing still reads the old table.
  ...
  record('Gate 4: LOOKUP_CATEGORIES gates the Finance categories too (migration 060)',
  ...
  const lookups = db.listFinanceLookups(userId);
  const seedOk = db.FINANCE_LOOKUP_CATEGORIES.every(cat => {
    const list = lookups.categories[cat] || [];
    return list.length === db.FINANCE_LOOKUP_SEED[cat].length
  ...
  // The catalog must actually be the shared table now, not a parallel one.
  const sharedConn = new DatabaseSync(path.join(workDir, 'cooperation-tools.db'));
  const inShared = ['CONTRACT_STATUS', 'CR_STATUS', 'INVOICE_STATUS', 'PAYMENT_METHOD']
    .map(cat => ({
      cat,
      n: sharedConn.prepare('SELECT COUNT(*) AS n FROM lookup_codes WHERE category = ?').get(cat).n,
    }));
  const dbSource = fs.readFileSync(path.join(__dirname, '..', 'db.js'), 'utf8');
  const noOldReads = !/(FROM|JOIN|INTO|UPDATE)\s+finance_lookups/.test(dbSource);
  sharedConn.close();
  record('Gate 5: the Finance categories live in the shared lookup_codes catalog',
    inShared.every(x => x.n > 0) && noOldReads,
    inShared.map(x => x.cat + '=' + x.n).join(', ') + '; db.js reads finance_lookups: ' + !noOldReads);
  ...
  const lookups2 = db.listFinanceLookups(secondUserId);
  record('Lookups: lazily seeded for a user created after the migration ran',
    (lookups2.categories.CURRENCY || []).length === db.FINANCE_LOOKUP_SEED.CURRENCY.length, '');

  // Setup tab: add a value, relabel an existing one, soft-disable another; codes stay immutable.
  const beforeSave = db.listFinanceLookups(userId).categories.PAYMENT_METHOD;
  const cashRow = beforeSave.find(o => o.code === 'CASH');
  const saveRes = db.saveFinanceLookups(userId, { categories: { PAYMENT_METHOD: [
    ...beforeSave.map(o => o.id === cashRow.id ? { ...o, labelEn: 'Cash Payment', isActive: false } : o),
    { labelEn: 'Wire Transfer', labelAr: 'حوالة' },
  ] } });
  const afterSave = db.listFinanceLookups(userId).categories.PAYMENT_METHOD;
  ...
  const clientRes = db.createFinanceClient(userId, { companyId: acmeCompanyId, contactEmail: 'ap@acme.test' });
  ...
  const listedClients = db.listFinanceClients(userId);
  ...
  record('Client: a rename in the shared catalog flows through to Finance',
    db.getFinanceClient(userId, clientId).name === 'Acme Corporation',
    db.getFinanceClient(userId, clientId).name);

  const updRes = db.updateFinanceClient(userId, clientId, { contactEmail: 'billing@acme.test', taxNumber: '300123' });
  ...
  const contractRes = db.createFinanceContract(userId, clientId, {
  ...
  const badCurrency = db.createFinanceContract(userId, clientId, { title: 'Bad currency test', currencyCode: 'XXX' });
  ...
  db.deleteFinanceContract(userId, badCurrency.contract.id);

  const v1 = db.createFinanceContractVersion(userId, contractId, { versionLabel: 'v1.0', valueMinor: 10_000_00, isFinal: true });
  const v2 = db.createFinanceContractVersion(userId, contractId, { versionLabel: 'v1.1', valueMinor: 12_000_00, isFinal: true });
  const v3 = db.createFinanceContractVersion(userId, contractId, { versionLabel: 'v1.2', valueMinor: 15_000_00 });
  const finalsAfterCreate = db.getFinanceContract(userId, contractId).versions.filter(v => v.isFinal);
  ...
  const v3Id = db.getFinanceContract(userId, contractId).versions.find(v => v.versionLabel === 'v1.2').id;
  db.setFinalFinanceContractVersion(userId, v3Id);
  db.setFinalFinanceContractVersion(userId, v3Id); // repeat call — must stay idempotent, still exactly one final
  const finalsAfterRepeat = db.getFinanceContract(userId, contractId).versions.filter(v => v.isFinal);
  ...
  const dupLabel = db.createFinanceContractVersion(userId, contractId, { versionLabel: 'v1.2', valueMinor: 1 });
  record('Version: duplicate version label on the same contract is rejected', dupLabel.ok === false, JSON.stringify(dupLabel));

  const inst1 = db.createFinanceInstallment(userId, contractId, { title: 'Kickoff', amountMinor: 5_000_00, dueDate: '2026-02-01' });
  const inst2 = db.createFinanceInstallment(userId, contractId, { title: 'Milestone 2', amountMinor: 5_000_00, dueDate: '2026-06-01' });
  record('Installment: auto-sequenced (1, then 2)', inst1.ok && inst2.ok
    && db.getFinanceContract(userId, contractId).installments.map(i => i.seq).join(',') === '1,2',
    JSON.stringify(db.getFinanceContract(userId, contractId).installments.map(i => i.seq)));

  const inst1Id = db.getFinanceContract(userId, contractId).installments[0].id;
  const dupSeq = db.createFinanceInstallment(userId, contractId, { seq: 1, title: 'Collides', amountMinor: 1 });
  record('Installment: duplicate seq on the same contract is rejected', dupSeq.ok === false, JSON.stringify(dupSeq));

  // ── Change Requests ──────────────────────────────────────────────────────────
  const crRes = db.createFinanceChangeRequest(userId, clientId, {
  ...
  const crForeignContract = db.createFinanceChangeRequest(userId, clientId, { title: 'x', contractId: 999999 });
  ...
  const dupRef = db.createFinanceChangeRequest(userId, clientId, { title: 'Dup ref', ref: 'CR-001' });
  record('CR: duplicate ref for the same user is rejected', dupRef.ok === false, JSON.stringify(dupRef));

  const listedCrs = db.listFinanceChangeRequests(userId, clientId);
  record('CR: list includes the created CR', listedCrs.some(c => c.id === crId), 'count=' + listedCrs.length);

  // ── Invoices + Links (invariants 2-4) ────────────────────────────────────────
  const invRes = db.createFinanceInvoice(userId, clientId, { number: 'INV-0001', amountMinor: 8_000_00, taxMinor: 0, currencyCode: 'sar', status: 'ISSUED' });
  ...
  const dupNumber = db.createFinanceInvoice(userId, clientId, { number: 'INV-0001', amountMinor: 1 });
  record('Invoice: duplicate number for the same user is rejected', dupNumber.ok === false, JSON.stringify(dupNumber));

  const usdInvoice = db.createFinanceInvoice(userId, clientId, { number: 'INV-USD-1', amountMinor: 100_00, currencyCode: 'usd' });
  const currencyMismatch = db.createFinanceInvoiceLink(userId, usdInvoice.invoice.id, { installmentId: inst1Id, allocatedMinor: 100 });
  ...
  db.deleteFinanceInvoice(userId, usdInvoice.invoice.id);

  const overAlloc = db.createFinanceInvoiceLink(userId, invoiceId, { installmentId: inst1Id, allocatedMinor: 999_999_00 });
  record('Link: over-allocating an installment beyond its amount is rejected', overAlloc.ok === false, JSON.stringify(overAlloc));

  const linkOk = db.createFinanceInvoiceLink(userId, invoiceId, { installmentId: inst1Id, allocatedMinor: 5_000_00 });
  record('Link: full allocation of an installment succeeds', linkOk.ok === true
    && db.getFinanceContract(userId, contractId).installments.find(i => i.id === inst1Id).outstandingMinor === 0,
    JSON.stringify(linkOk));

  const dupLink = db.createFinanceInvoiceLink(userId, invoiceId, { installmentId: inst1Id, allocatedMinor: 1 });
  ...
  const exclusivity = db.createFinanceInvoiceLink(userId, invoiceId, {});
  record('Link: neither installment nor CR specified is rejected', exclusivity.ok === false, JSON.stringify(exclusivity));

  const bothSpecified = db.createFinanceInvoiceLink(userId, invoiceId, { installmentId: inst1Id, crId, allocatedMinor: 1 });
  ...
  const refusedInstDelete = db.deleteFinanceInstallment(userId, inst1Id);
  record('Installment: delete refused once invoiced, row untouched', refusedInstDelete.ok === false
    && db.getFinanceContract(userId, contractId).installments.some(i => i.id === inst1Id), JSON.stringify(refusedInstDelete));

  const crOverAlloc = db.createFinanceInvoiceLink(userId, invoiceId, { crId, allocatedMinor: 999_999_00 });
  record('Link: over-allocating a CR beyond its amount is rejected', crOverAlloc.ok === false, JSON.stringify(crOverAlloc));
  const crLinkOk = db.createFinanceInvoiceLink(userId, invoiceId, { crId, allocatedMinor: 3_000_00 });
  record('Link: full allocation of a CR succeeds', crLinkOk.ok === true, JSON.stringify(crLinkOk));
  const refusedCrDelete = db.deleteFinanceChangeRequest(userId, crId);
  ...
  const payTooMuch = db.createFinancePayment(userId, invoiceId, { amountMinor: invoiceTotal + 1, method: 'BANK_TRANSFER' });
  record('Payment: over-paying the invoice is rejected', payTooMuch.ok === false, JSON.stringify(payTooMuch));

  const partialPay = db.createFinancePayment(userId, invoiceId, { amountMinor: 5_000_00, method: 'bank_transfer', paidDate: '2026-03-01' });
  ...
  const remainderTooMuch = db.createFinancePayment(userId, invoiceId, { amountMinor: 3_000_01 });
  ...
  const finalPay = db.createFinancePayment(userId, invoiceId, { amountMinor: 3_000_00 });
  ...
  const payUpdateOverpay = db.updateFinancePayment(userId, paymentId, { amountMinor: 3_000_01 });
  ...
  const payDelete = db.deleteFinancePayment(userId, paymentId);
  ...
  const invoiceDelete = db.deleteFinanceInvoice(userId, invoiceId);
  ...
  const instFreedNow = db.getFinanceContract(userId, contractId).installments.find(i => i.id === inst1Id);
  ...
  const crDeleteAllowedNow = db.deleteFinanceChangeRequest(userId, crId);
  ...
  const allowedDelete = db.deleteFinanceInstallment(userId, inst1Id);
  ...
  const clientDeleteRefused = db.deleteFinanceClient(userId, clientId);
  ...
  const someVersionId = db.getFinanceContract(userId, contractId).versions[0].id;
  db.deleteFinanceContractVersion(userId, someVersionId);
  const remainingVersionsBeforeDelete = db.getFinanceContract(userId, contractId).versions.length;
  db.deleteFinanceContract(userId, contractId);
  const contractGone = db.getFinanceContract(userId, contractId) === null;
  ...
  const clientDeleteAllowed = db.deleteFinanceClient(userId, clientId);
  record('Client: delete allowed once contract-free', clientDeleteAllowed.ok === true, JSON.stringify(clientDeleteAllowed));

  // ── Phase 3: Attachments ─────────────────────────────────────────────────────
  const p3Client = db.createFinanceClient(userId, { companyId: newCompany('Phase 3 Client') }).client;
  const p3ContractRes = db.createFinanceContract(userId, p3Client.id, { title: 'Phase 3 Contract', status: 'ACTIVE' });
  const p3VersionRes = db.createFinanceContractVersion(userId, p3ContractRes.contract.id, { versionLabel: 'v1.0', valueMinor: 1_000_00, isFinal: true });
  ...
  const unsupportedExt = db.createFinanceAttachment(userId, 'contract_version', p3VersionId, unsupportedPath);
  record('Attachment: unsupported extension rejected', unsupportedExt.ok === false, JSON.stringify(unsupportedExt));

  const spoofedContent = db.createFinanceAttachment(userId, 'contract_version', p3VersionId, spoofedPngPath);
  ...
  const foreignEntity = db.createFinanceAttachment(userId, 'contract_version', 999999, goodPngPath);
  record('Attachment: unowned/unknown entity is rejected', foreignEntity.ok === false, JSON.stringify(foreignEntity));

  const attachRes = db.createFinanceAttachment(userId, 'contract_version', p3VersionId, goodPngPath);
  record('Attachment: valid upload succeeds and copies the file', attachRes.ok === true
    && fs.existsSync(db.resolveFinanceAttachment(userId, attachRes.attachment.id).absPath),
    JSON.stringify(attachRes));
  const attachmentId = attachRes.attachment.id;

  const listedAttachments = db.listFinanceAttachments(userId, 'contract_version', p3VersionId);
  ...
  if (otherUserRow) {
    const stolenList = db.listFinanceAttachments(otherUserRow.id, 'contract_version', p3VersionId);
    const stolenResolve = db.resolveFinanceAttachment(otherUserRow.id, attachmentId);
  ...
  const resolved = db.resolveFinanceAttachment(userId, attachmentId);
  record('Attachment: resolve returns the file on disk', resolved.ok === true && resolved.exists === true, JSON.stringify(resolved));

  const attachDelete = db.deleteFinanceAttachment(userId, attachmentId);
  ...
  record('Attachment: delete removes the row but leaves the file for the undo window', attachDelete.ok === true
    && fileStillOnDiskAfterDelete && db.listFinanceAttachments(userId, 'contract_version', p3VersionId).length === 0,
    JSON.stringify({ attachDelete, fileStillOnDiskAfterDelete }));

  const restoreRes = db.restoreFinanceAttachment(userId, attachDelete.snapshot);
  record('Attachment: restore re-creates the row against the same file', restoreRes.ok === true
    && db.listFinanceAttachments(userId, 'contract_version', p3VersionId).length === 1,
    JSON.stringify(restoreRes));

  const attachDelete2 = db.deleteFinanceAttachment(userId, restoreRes.attachment.id);
  const purgeSnap = attachDelete2.snapshot;
  if (otherUserRow) {
    db.purgeFinanceAttachmentFile(otherUserRow.id, purgeSnap.entityType, purgeSnap.entityId, purgeSnap.filePath);
  ...
  db.purgeFinanceAttachmentFile(userId, purgeSnap.entityType, purgeSnap.entityId, purgeSnap.filePath);
  ...
  const attachForPurge = db.createFinanceAttachment(userId, 'contract_version', p3VersionId, goodPngPath);
  const purgeAbsPath = db.resolveFinanceAttachment(userId, attachForPurge.attachment.id).absPath;
  db.deleteFinanceContractVersion(userId, p3VersionId);
  record('Attachment: deleting the owning contract version purges its attachments immediately',
    !fs.existsSync(purgeAbsPath) && db.listFinanceAttachments(userId, 'contract_version', p3VersionId).length === 0,
    'purgeAbsPath=' + purgeAbsPath);

  // ── Phase 3: Minutes of Meeting ──────────────────────────────────────────────
  const meetingForeignContract = db.createFinanceMeeting(userId, p3Client.id, { title: 'x', contractId: 999999 });
  ...
  const meetingRes = db.createFinanceMeeting(userId, p3Client.id, {
  ...
  const meetingNoTitle = db.createFinanceMeeting(userId, p3Client.id, { title: '' });
  record('Meeting: blank title is rejected', meetingNoTitle.ok === false, JSON.stringify(meetingNoTitle));

  const listedMeetings = db.listFinanceMeetings(userId, p3Client.id);
  record('Meeting: list includes the created meeting', listedMeetings.some(m => m.id === meetingId), 'count=' + listedMeetings.length);

  const meetingUpdRes = db.updateFinanceMeeting(userId, meetingId, { title: 'Kickoff meeting (revised)', content: '<p>updated</p>' });
  ...
  if (otherUserRow) {
    const stolenMeeting = db.getFinanceMeeting(otherUserRow.id, meetingId);
    record('Meeting: ownership gating on read', stolenMeeting === null, JSON.stringify(stolenMeeting));
  }

  const actionNoDesc = db.createFinanceMeetingAction(userId, meetingId, { description: '' });
  record('Action item: blank description is rejected', actionNoDesc.ok === false, JSON.stringify(actionNoDesc));

  const action1 = db.createFinanceMeetingAction(userId, meetingId, { description: 'Send proposal', owner: 'Alice', dueDate: '2026-08-10' });
  const action2 = db.createFinanceMeetingAction(userId, meetingId, { description: 'Schedule follow-up', owner: 'Bob' });
  ...
  const toggled = db.toggleFinanceMeetingActionStatus(userId, action1Id);
  const toggledBack = db.toggleFinanceMeetingActionStatus(userId, action1Id);
  ...
  const actionDelete = db.deleteFinanceMeetingAction(userId, action2Id);
  ...
  const meetingDelete = db.deleteFinanceMeeting(userId, meetingId);
  record('Meeting: delete cascades its action items', meetingDelete.ok === true
    && meetingDelete.snapshot.actions.length === 1 && db.getFinanceMeeting(userId, meetingId) === null,
  ...
  const p3bClient = db.createFinanceClient(userId, { companyId: newCompany('Cascade Guard Client') }).client;

  const p3bCr = db.createFinanceChangeRequest(userId, p3bClient.id, { title: 'Standalone CR', amountMinor: 100_00 });
  const crRefusal = db.deleteFinanceClient(userId, p3bClient.id);
  record('Client: delete refused while it still owns a standalone change request', crRefusal.ok === false, JSON.stringify(crRefusal));
  db.deleteFinanceChangeRequest(userId, p3bCr.cr.id);

  const p3bInvoice = db.createFinanceInvoice(userId, p3bClient.id, { number: 'INV-P3B-1', amountMinor: 100_00 });
  const invoiceRefusal = db.deleteFinanceClient(userId, p3bClient.id);
  ...
  db.deleteFinanceInvoice(userId, p3bInvoice.invoice.id);

  const p3bMeeting = db.createFinanceMeeting(userId, p3bClient.id, { title: 'Standalone meeting' });
  const meetingRefusal = db.deleteFinanceClient(userId, p3bClient.id);
  ...
  db.deleteFinanceMeeting(userId, p3bMeeting.meeting.id);

  const p3bDeleteAllowed = db.deleteFinanceClient(userId, p3bClient.id);
  ...
  const cxClient = db.createFinanceClient(userId, { companyId: newCompany('Cancel Test Client') }).client;
  const cxContract = db.createFinanceContract(userId, cxClient.id, { title: 'Cancel Test Contract', status: 'ACTIVE' }).contract;
  db.createFinanceInstallment(userId, cxContract.id, { title: 'Only', amountMinor: 1_000_00 });
  const cxInstId = db.getFinanceContract(userId, cxContract.id).installments[0].id;
  const cxInv = db.createFinanceInvoice(userId, cxClient.id, { number: 'INV-CX-1', amountMinor: 1_000_00, status: 'ISSUED' }).invoice;
  db.createFinanceInvoiceLink(userId, cxInv.id, { installmentId: cxInstId, allocatedMinor: 1_000_00 });

  const cxBlockedInstDelete = db.deleteFinanceInstallment(userId, cxInstId);
  ...
  const cxBlockedContractDelete = db.deleteFinanceContract(userId, cxContract.id);
  ...
  const cxCancelRes = db.updateFinanceInvoice(userId, cxInv.id, { number: 'INV-CX-1', amountMinor: 1_000_00, status: 'CANCELLED' });
  ...
  const cxInstAfterCancel = db.getFinanceContract(userId, cxContract.id).installments.find(i => i.id === cxInstId);
  ...
  const cxSummary = db.getFinanceClientSummary(userId, cxClient.id);
  record('Summary: a cancelled invoice is excluded from invoicedMinor', cxSummary.invoicedMinor === 0, JSON.stringify(cxSummary));

  const cxAllowedInstDelete = db.deleteFinanceInstallment(userId, cxInstId);
  ...
  const cxAllowedContractDelete = db.deleteFinanceContract(userId, cxContract.id);
  ...
  db.deleteFinanceInvoice(userId, cxInv.id);
  db.deleteFinanceClient(userId, cxClient.id);

  // ── Summary ───────────────────────────────────────────────────────────────
  const summaryClient = db.createFinanceClient(userId, { companyId: newCompany('Summary Test Client') }).client;
  db.createFinanceContract(userId, summaryClient.id, { title: 'C1', status: 'ACTIVE' });
  const summary = db.getFinanceClientSummary(userId, summaryClient.id);
  ...
  const feedClient = db.createFinanceClient(userId, { companyId: newCompany('Feed Client', 'FEED') }).client;
  const feedContract = db.createFinanceContract(userId, feedClient.id, {
    title: 'Feed Contract', status: 'ACTIVE', endDate: '2026-12-31',
  }).contract;

  const attentionOf = (type) => db.getFinanceAttentionItems(userId).filter(a => a.type === type);
  ...
  db.updateFinanceContract(userId, feedContract.id, {
  ...
  const noDate = db.createFinanceContract(userId, feedClient.id, { title: 'No End Date', status: 'ACTIVE' }).contract;
  record('Attention: a contract with no end date is never surfaced',
    !attentionOf('financeContract').some(a => a.id === noDate.id));

  const feedInvoice = db.createFinanceInvoice(userId, feedClient.id, {
  ...
  db.createFinancePayment(userId, feedInvoice.id, { amountMinor: 50000, paidDate: '2026-11-01' });
  ...
  const cancelled = db.createFinanceInvoice(userId, feedClient.id, {
  ...
  const otherUserAttention = db.getFinanceAttentionItems(secondUserId);
  ...
  const activity = db.getFinanceRecentActivity(userId, 20);
  ...
  record('Activity: newest first and obeys its limit',
    db.getFinanceRecentActivity(userId, 2).length <= 2
      && activity.every((a, i) => i === 0 || activity[i - 1].changedAt >= a.changedAt));
  record('Activity: never leaks across accounts',
    !db.getFinanceRecentActivity(secondUserId, 20).some(a => a.parentId === feedClient.id));
  ...
  const curClient = db.createFinanceClient(userId, { companyId: newCompany('Currency Co') }).client;
  const sharedCurrency = (db.getLookupsByCategory('CURRENCY', false)[0] || {}).code;
  const sharedContract = db.createFinanceContract(userId, curClient.id, {
  ...
  const ownContract = db.createFinanceContract(userId, curClient.id, {
  ...
  const junkContract = db.createFinanceContract(userId, curClient.id, {
  ...
  const ovClient = db.createFinanceClient(userId, { companyId: newCompany('Overview Co') }).client;
  db.createFinanceContract(userId, ovClient.id, { title: 'Live', status: 'ACTIVE' });
  const ovPaid = db.createFinanceInvoice(userId, ovClient.id, {
    number: 'OV-PAID', amountMinor: 10000, dueDate: '2020-01-01', status: 'ISSUED',
  }).invoice;
  db.createFinancePayment(userId, ovPaid.id, { amountMinor: 10000, paidDate: '2020-01-02' });
  db.createFinanceInvoice(userId, ovClient.id, {
  ...
  const ov = db.getFinanceOverview(userId);
  ...
  record('Overview: never leaks across accounts',
    db.getFinanceOverview(secondUserId).invoicedMinor === 0,
    JSON.stringify(db.getFinanceOverview(secondUserId)));
