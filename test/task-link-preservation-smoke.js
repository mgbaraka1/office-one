...
//
// Gates exercised:
//   1. updateTaskMeta() on a DEPARTMENT-linked task, called with the exact
//      link-less payload the Timesheet reconciler sends, leaves
//      department_id completely unchanged.
...
//      (name/status/company/system/source).
...
//      change project_id/department_id, and records it in task_field_history
//      by human label (project name / department label), not raw id.
// ─────────────────────────────────────────────────────────────────────────────
...
try {
  ...
  // The exact shape persistTimesheet()'s tsTaskPayload(row) builds — no
  // department field at all, and a projectId that updateTaskMeta must ignore.
  const reconcilerPayload = (overrides) => Object.assign({
    name: 'unchanged name', status: 'IN_PROGRESS', company: '', system: '', source: '',
    projectId: null,
  }, overrides);

  // ── Gate 1 — department link survives updateTaskMeta ────────────────────
  {
    const t = db.createTask(userId, { name: 'Dept task', status: 'OPEN', department: dept.label });
    ...
    record('Gate 1: updateTaskMeta() leaves a DEPARTMENT-linked task\'s departmentId unchanged',
      after.departmentId === before.departmentId && after.departmentId === dept.id,
      `before=${before.departmentId} after=${after.departmentId}`);
  }
  ...
  let metaTaskId;
  {
    const t = db.createTask(userId, { name: 'Meta task', status: 'OPEN', department: dept.label });
  ...
  // ── Gate 6 — the full updateTask() (real link editors) still changes links,
  //             and records the change by human label, not raw id ───────────
  {
    const t = db.createTask(userId, { name: 'Link editor task', status: 'OPEN', department: dept.label });
    const before = db.getTaskFieldHistory(userId, t.id);
    // Full editor round-trip: unlink department, link project instead — the
    // exact shape openBacklogModal's own payload takes.
    db.updateTask(userId, t.id, { name: t.name, status: 'OPEN', company: '', system: '', source: '', projectId: project.id, department: '' });
    const after = db.getTask(userId, t.id);
    ...
    const pass = after.projectId === project.id && after.departmentId === null
      ...
      && !!deptRow && deptRow.oldValue === dept.label && deptRow.newValue === ''
      && !!projRow && projRow.oldValue === '' && projRow.newValue === project.name;
    record('Gate 6: updateTask() (link editors) still changes project/department links, audited by human label',
  ...
  {
    const t = db.createTask(userId, { name: 'Legacy-source task', status: 'OPEN', department: dept.label, source: 'LEGACY-JIRA-ABC-1' });
    // The exact shape submitBacklogModal's payload takes — no `source` key.
    db.updateTask(userId, t.id, { name: t.name, status: 'IN_PROGRESS', company: '', system: '', department: dept.label });
    const after = db.getTask(userId, t.id);
  ...
  // ── Gate 8 — updateTaskMeta() has the same belt-and-braces guard ──────────
  {
    const t = db.createTask(userId, { name: 'Legacy-source meta task', status: 'OPEN', department: dept.label, source: 'LEGACY-EMAIL-SUBJECT' });
    ...
    db.updateTaskMeta(userId, t.id, { name: t.name, status: 'BLOCKED', company: '', system: '' });
    const after = db.getTask(userId, t.id);
