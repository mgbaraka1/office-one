...
const LOOKUP_CATEGORIES = ['COMPANY', 'SYSTEM', 'ACTIVITY_TYPE', 'TIME_TYPE', 'ENTRY_STATUS', 'CURRENCY', 'BILLING_CYCLE', 'PROJECT_STATUS', 'PROJECT_DOCUMENT', 'COMPANY_DOCUMENT_CATEGORY', 'KNOWLEDGE_TYPE', 'DEPARTMENT', 'TASK_SOURCE_TYPE', 'SERVER_ROLE',
  // Finance's statuses and payment methods, folded in from its own catalog by
  // migration 060. Edited in Settings → Finance, so their registry entries are
  // settingsTab: false / editor: 'external' — the same shape COMPANY uses.
...
// Returns fn()'s value, so a write helper can hand back the row it created.
let txDepth = 0;
function tx(fn) {
...
// `app_settings` has no app-layer caller today — the migrations that seed it
// read and write the table directly. This pair stays so all three K/V scopes
// (app / machine / user) are reachable the same way; see CONTRIBUTING.md.
...
// Exposes the live connection to the Finance section further down this file
// (see ARCHITECTURE.md) so it shares this same
...
// the Finance section's own delete paths already purge attachments immediately on a
...
// ─── Migration 035 (Sub-Projects / Annual Support) — RETIRED ────────────────
//
// 035 added a project hierarchy (`projects.parent_project_id`) and an Annual
// Support model (`project_support_years`, `tasks.support_year_id`). The whole
// feature was retired on 2026-07-09: nothing creates a sub-project or a support
// year, and no write path sets either link.
//
// The SCHEMA could not follow, and that is deliberate:
//   • migrations are append-only, so the columns and the table stay;
//   • migration 048's trg_tasks_link_exclusive_* triggers still name
//     `support_year_id`, so the task write paths further down MUST keep passing
//     that column through. Dropping it from an INSERT/UPDATE would put the app
//     out of step with a constraint SQLite is still enforcing.
//
// So the two resolvers below still validate a link nothing supplies, and
// `support_year_id` still travels through every task write. That is residue,
// not a live feature. Treat anything naming `parent_project_id`,
// `project_support_years` or `support_year_id` as frozen: don't extend it, and
// don't tidy it away either. See "Retired surfaces" in CONTRIBUTING.md.
...
// a sub-project can never itself gain sub-projects. Retired — nothing calls
// this with a non-null parentProjectId any more, so it always returns null.
...
// null. Retired (migration 035): no UI produces a supportYearId, so in practice
// this only ever returns null — but see the note above, the column stays.
...
// the list view's payload. taskCount is the number of TASKS linked directly to
// the project; each may have zero or many work sessions.
//
// The `parent_project_id IS NULL` filter is migration 035 residue (see the note
// above resolveParentProjectId). Nothing sets that column now, so it matches
// every row; it stays so a database still holding a pre-2026-07 sub-project
// keeps rendering the flat grid the UI expects.
...
// The two halves of the client/internal split (migration 053). Only the client
// side has a caller right now; the pair is kept together so the predicate is
// never re-derived by hand on the other side. See CONTRIBUTING.md.
// eslint-disable-next-line no-unused-vars
...
// missing source, so without a map here a Full Restore of an older bundle would
// succeed while quietly dropping every file under the renamed directory. Same
// spirit as FULL_BACKUP_PREFIXES accepting the pre-rebrand folder prefix: old
// backups must keep restoring.
//
// Currently empty — no upload directory in a restorable bundle carries an old
// name. Add an entry here rather than renaming a directory silently.
const FULL_BACKUP_LEGACY_DIRS = {};
...
function restoreFullBackup(bundleDir, options = {}) {
  ...
  // Only current names are installed from the stage, but a live directory could
  // still be sitting under a legacy name (FULL_BACKUP_LEGACY_DIRS). Move those
  // aside too, or a stale tree would survive the restore and its files would
  // look like orphans afterwards.
...
// FINANCE
//
// Financial record-keeping: clients, contracts with versions, change requests,
// invoices with payment tracking, and minutes of meeting. Formerly a separate
// module (finance-db.js) that shared this file's connection; folded in here so
// there is one data layer rather than two.
//
// It still owns 100% of its own SQL and its own tables (all `finance_`
// prefixed, all user_id-scoped). Two conventions differ from the code above
// and are deliberate:
//
//   • Money is INTEGER MINOR UNITS (halalas/cents) on every amount column,
//     unlike subscriptions.cost REAL. Invoice-to-installment reconciliation
//     needs exact integer arithmetic.
//   • Every mutating function returns { ok, ... } rather than throwing, so a
//     refusal is never a partial write.
//
// See ARCHITECTURE.md for the cross-entity invariants this section enforces.
// ─────────────────────────────────────────────────────────────────────────────

// Finance shares the one connection opened by openConnection() above.
function conn() { return getConnection(); }
...
// ── Finance's catalog ────────────────────────────────────────────────────────
// Finance used to keep its own user_id-scoped `finance_lookups` table. Migration
// 060 folded it into the shared, global `lookup_codes`, so these categories are
// now ordinary catalog rows: gated by LOOKUP_CATEGORIES, audited in
// lookup_code_history like every other catalog edit, and no longer per-account.
//
// They keep a dedicated editor (Settings → Finance) rather than a shared
// catalog tab, which is why their registry entries are settingsTab: false.
// Codes remain the immutable identity; the labels are freely editable.
//
// CURRENCY is listed here so the Finance pickers can offer it, but it is the
// app-wide CURRENCY catalog — the same list Subscriptions uses. `currency_code`
// stores that code as a string, never an id.
// FINANCE_LOOKUP_SEED is retained as the fresh-install fallback and as the
// canonical vocabulary; migration 054 holds its own frozen copy.
...
// Safety net for a catalog that ended up empty — normally migration 060 has
// already carried these categories across, and on a fresh install 054 seeds
// them before 060 moves them. Guarded per category on the category being
// empty, so it is a verified no-op on a curated catalog (the same rule
// migration 059 follows). CURRENCY is skipped: the app-wide catalog owns it.
// `c` is an explicit connection param so a caller can pass the connection it
// is already holding.
function seedLookupsIfMissing(c) {
  const now = nowIso();
  const countIn = c.prepare('SELECT COUNT(*) AS n FROM lookup_codes WHERE category = ?');
  const ins = c.prepare(
  ...
  for (const [category, items] of Object.entries(FINANCE_LOOKUP_SEED)) {
    ...
    items.forEach(([code, labelEn, labelAr], i) => ins.run(category, code, labelEn, i, now, labelEn, labelAr));
  }
}
...
function finUniqueCode(category, base) {
  const exists = conn().prepare('SELECT 1 FROM lookup_codes WHERE category = ? AND code = ?');
  let code = base, i = 2;
  while (exists.get(category, code)) code = base + '_' + (i++);
  return code;
}
...
function finLkId(category, code) {
  const c = String(code ?? '').trim().toUpperCase();
  if (!c) return null;
  const row = conn().prepare('SELECT id FROM lookup_codes WHERE category = ? AND code = ?').get(category, c);
  return row ? row.id : null;
}
function finLkFields(statusId) {
  if (statusId == null) return { status: '', statusLabelEn: '', statusLabelAr: '' };
  const r = conn().prepare(
    `SELECT code, COALESCE(NULLIF(name_en, ''), label) AS label_en, name_ar AS label_ar
       FROM lookup_codes WHERE id = ?`
  ).get(statusId);
  return r ? { status: r.code, statusLabelEn: r.label_en, statusLabelAr: r.label_ar || '' } : { status: '', statusLabelEn: '', statusLabelAr: '' };
}
// Currency is stored as a plain code, not an FK id, and resolves against the
// app-wide CURRENCY catalog — the only one there is since migration 060, which
// also carried across any code Finance knew that the shared list did not, so no
// previously stored code became invalid. An unknown or blank code resolves to
// '' (unset) rather than storing junk.
function resolveFinanceCurrency(code) {
  const c = String(code ?? '').trim().toUpperCase();
  if (!c) return '';
...
function listFinanceLookups(userId) {
  seedLookupsIfMissing(conn());
  // Global since migration 060 — userId is kept on the signature because the
  // IPC layer passes the session's id to every db.* call uniformly.
  void userId;
  const rows = conn().prepare(
    `SELECT id, category, code, sort_order, is_active,
            COALESCE(NULLIF(name_en, ''), label) AS label_en,
            COALESCE(name_ar, '') AS label_ar
       FROM lookup_codes WHERE category IN (${FINANCE_LOOKUP_CATEGORIES.map(() => '?').join(', ')})
      ORDER BY category, sort_order, id`
  ).all(...FINANCE_LOOKUP_CATEGORIES);
  const categories = {};
...
function saveFinanceLookups(userId, data) {
  ...
  tx(() => {
    const c = conn();
    const now = nowIso();
    const readBefore = c.prepare('SELECT label, name_en, name_ar, is_active FROM lookup_codes WHERE id = ?');
    const upd = c.prepare('UPDATE lookup_codes SET label = ?, name_en = ?, name_ar = ?, sort_order = ?, is_active = ? WHERE id = ?');
    const ins = c.prepare('INSERT INTO lookup_codes(category, code, label, sort_order, is_active, created_at, name_en, name_ar) VALUES (?,?,?,?,?,?,?,?)');
    for (const [cat, list] of Object.entries(data.categories)) {
      ...
      list.forEach((item, i) => {
        ...
        const active = item?.isActive === false ? 0 : 1;
        if (itemId != null) {
          const before = c.prepare('SELECT 1 FROM lookup_codes WHERE id = ? AND category = ?').get(itemId, cat)
            ? readBefore.get(itemId) : null;
          if (!before) { skipped.push({ category: cat, reason: 'not-found', index: i }); return; }
          upd.run(labelEn, labelEn, labelAr, sort, active, itemId);
          // Shared catalog, shared audit trail (migration 058).
          recordLookupHistory(userId, itemId, cat, before,
            { label: labelEn, name_en: labelEn, name_ar: labelAr, is_active: active });
        } else {
          const code = finUniqueCode(cat, finSlugCode(labelEn));
          const newId = Number(ins.run(cat, code, labelEn, sort, active, now, labelEn, labelAr).lastInsertRowid);
          recordLookupHistory(userId, newId, cat, null,
            { label: labelEn, name_en: labelEn, name_ar: labelAr, is_active: active });
        }
      });
    }
...
const FINANCE_CLIENT_SELECT = `
  ...
  COALESCE((
    ...
    LEFT JOIN lookup_codes st ON st.id = i.status_id
...
function companyForFinance(userId, companyId) {
  ...
  return canAccessLookup(userId, id) ? row : null;
}
...
function listFinanceCandidateCompanies(userId) {
  ...
  return conn().prepare(
    ...
    .filter(r => !taken.has(r.id) && canAccessLookup(userId, r.id))
  ...
  const id = Number(conn().prepare(
    `INSERT INTO finance_contracts(user_id, client_id, ref, title, description, status_id, currency_code, start_date, end_date, notes, created_at, updated_at)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`
        ...
        finLkId('CONTRACT_STATUS', data?.status), resolveFinanceCurrency(data?.currencyCode),
...
function createFinanceChangeRequest(userId, clientId, data) {
  ...
  const contractId = data?.contractId != null ? Number(data.contractId) : null;
  if (contractId != null) {
    ...
    const id = Number(conn().prepare(
      `INSERT INTO finance_change_requests(user_id, client_id, contract_id, ref, title, description, status_id, amount_minor, currency_code, requested_date, approved_date, notes, created_at, updated_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
          ...
          finLkId('CR_STATUS', data?.status), amountMinor, resolveFinanceCurrency(data?.currencyCode),
    ...
    const id = Number(conn().prepare(
      `INSERT INTO finance_invoices(user_id, client_id, number, issue_date, due_date, amount_minor, tax_minor, currency_code, status_id, notes, created_at, updated_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`
          ...
          resolveFinanceCurrency(data?.currencyCode), finLkId('INVOICE_STATUS', data?.status),
...
function updateFinanceInvoice(userId, id, data) {
  ...
  const amountMinor = Math.max(0, Math.round(Number(data?.amountMinor) || 0));
  const taxMinor = Math.max(0, Math.round(Number(data?.taxMinor) || 0));
  const statusId = finLkId('INVOICE_STATUS', data?.status);
      ...
      conn().prepare(
        `UPDATE finance_invoices SET number = ?, issue_date = ?, due_date = ?, amount_minor = ?, tax_minor = ?, currency_code = ?,
           status_id = ?, notes = ?, updated_at = ? WHERE id = ? AND user_id = ?`
            ...
            resolveFinanceCurrency(data?.currencyCode), statusId,
...
function createFinancePayment(userId, invoiceId, data) {
  ...
  conn().prepare(
    `INSERT INTO finance_invoice_payments(user_id, invoice_id, paid_date, amount_minor, method_id, reference, notes, created_at)
     VALUES (?,?,?,?,?,?,?,?)`
  ).run(userId, invoiceId, data?.paidDate || null, amountMinor, finLkId('PAYMENT_METHOD', data?.method),
...
function updateFinancePayment(userId, id, data) {
  ...
  ).run(data?.paidDate || null, amountMinor, finLkId('PAYMENT_METHOD', data?.method),
...
function getFinanceClientSummary(userId, clientId) {
  ...
  const activeContractCount = c.prepare(
    `SELECT COUNT(*) AS n FROM finance_contracts k
       LEFT JOIN lookup_codes st ON st.id = k.status_id
...
// this avoids what used to be a circular require — the Finance code lived in
    ...
    `SELECT n.id, n.title, n.seq, n.due_date, k.client_id, c.company_id,
       ...
       LEFT JOIN (
         SELECT il.installment_id, SUM(il.allocated_minor) AS allocated
           ...
           LEFT JOIN lookup_codes s ON s.id = iv.status_id
...
module.exports = {
  ...
  listFinanceLookups, saveFinanceLookups, listFinanceClients, getFinanceClient,
  createFinanceClient, updateFinanceClient, deleteFinanceClient,
  listFinanceCandidateCompanies, listFinanceContracts, getFinanceContract,
  createFinanceContract, updateFinanceContract, deleteFinanceContract,
  createFinanceContractVersion, updateFinanceContractVersion, deleteFinanceContractVersion,
  setFinalFinanceContractVersion, createFinanceInstallment, updateFinanceInstallment,
  deleteFinanceInstallment, listFinanceChangeRequests, getFinanceChangeRequest,
  createFinanceChangeRequest, updateFinanceChangeRequest, deleteFinanceChangeRequest,
  listFinanceInvoices, getFinanceInvoice, createFinanceInvoice, updateFinanceInvoice,
  deleteFinanceInvoice, createFinanceInvoiceLink, deleteFinanceInvoiceLink,
  createFinancePayment, updateFinancePayment, deleteFinancePayment, FINANCE_DOC_EXTENSIONS,
  ...
  listFinanceMeetings, getFinanceMeeting, createFinanceMeeting, updateFinanceMeeting,
  deleteFinanceMeeting, createFinanceMeetingAction, updateFinanceMeetingAction,
  toggleFinanceMeetingActionStatus, deleteFinanceMeetingAction, getFinanceClientSummary,
  LOOKUP_CATEGORIES, LOOKUP_MERGE_TARGETS, DB_FILENAME, USER_DATA_ENTRIES,
  USER_DATA_KEY_ENTRY, carryOverLegacyUserData, openConnection, applyMigrations,
  runMaintenance, getConnection, close, backup, dbPath, projectsRootDir, knowledgeRootDir,
  countUsers, getUserByUsername, getUserById, listUsers, countActiveAdmins,
  countActiveUsers, createUser, updateUserAccount, getUnclaimedUser, claimUser, listDays,
  loadDaysRange, listCompanies, listSystems, companyEntries, systemEntries,
  getFilteredWorkLogs, getAnalytics, getOverviewStats, getAttentionItems,
  getRecentActivity, createProject, listProjects, getProject, updateProject, deleteProject,
  linkTask, unlinkTask, listLinkableTasks, listDepartments, getDepartment,
  listInternalTasks, createInternalTask, updateInternalTask, convertTaskToInternal,
  convertTaskToClient, listTasks, getTasksIndex, getTask, searchWorkspace, createTask,
  updateTask, updateTaskMeta, deleteTask, getTaskSources, createTaskSource,
  updateTaskSource, deleteTaskSource, getTaskFieldHistory, listWorkLogs, logsForDate,
  addWorkLog, updateWorkLog, moveWorkLog, mergeTasks, deleteWorkLog, getWorkLogHistory,
  setDayName, getDayName, saveProjectDocumentFile, resolveProjectDocumentFile,
  removeProjectDocumentFile, restoreProjectDocumentFile,
  purgeUnreferencedProjectDocumentFile, purgeProjectFiles, restoreProjectFiles,
  listCompanyDocuments, getCompanyDocument, createCompanyDocument, updateCompanyDocument,
  deleteCompanyDocument, saveCompanyDocumentFile, resolveCompanyDocumentFile,
  removeCompanyDocumentFile, restoreRemovedCompanyDocumentFile,
